...
class NewPOForm(forms.ModelForm):
    ...
    billing_receiver = forms.ModelChoiceField(queryset=CompanyMaintenance.objects.filter(is_active=True).order_by('company_name'), label="Billing Address", empty_label="Not Assigned",required=False)
    ...
    vendor_address = forms.CharField(widget=forms.Textarea,required=False)
...
class UpdatePOForm(forms.ModelForm):
    ...
    billing_receiver = forms.ModelChoiceField(queryset=CompanyMaintenance.objects.filter(is_active=True).order_by('company_name'), label="Billing Address", empty_label="Not Assigned", initial=PurchaseOrder.billing_receiver,required=False)
